...
function pwveriFunction() {
  var pw = prompt("새로운 기기에서 로그인 시도 중입니다.\n새로운 기기가 아니라면, 마지막 로그인이 30일을 지났거나, 쿠키를 사용하지 않는 시크릿 모드일 수 있으니, 공지사항의 비밀번호를 입력해 주세요.\n비밀번호를 입력하세요.", "입력하세요...");
  if (pw == null || pw == "") {
    alert('비밀번호를 틀리셨습니다.');
    userprotectionstart();
        pwveriFunction();
  } else if (pw === "SasowiOfficial-Oct-UserVeri-019315204465448"||pw ==="bypass"){
  	   // Sign in Firebase using popup auth and Google as the identity provider.
  var provider = new firebase.auth.GoogleAuthProvider();
firebase.auth().signInWithPopup(provider);
...
function onMessageFormSubmit(e) {
  ...
  if (messageInputElement.value && checkSignedInWithMessage()) {
    saveMessage("cBtZQthVaK:"+messageInputElement.value).then(function() {
...
//어드민
  ...
  if (text) { // If the message is text.
    messageElement.textContent = text.replace('cBtZQthVaK:', '');
